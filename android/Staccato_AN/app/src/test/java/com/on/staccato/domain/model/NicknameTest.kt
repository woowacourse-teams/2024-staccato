...
class NicknameTest {
    ...
    fun `닉네임은 한글, 영문, 숫자, 마침표, 밑줄, 띄어쓰기로 조합할 수 있다`(nickname: String) {
        // when
        val nicknameState = NicknameState.validate(nickname)
    ...
    fun `닉네임이 빈 문자열이라면 비어있는 상태이다`() {
        // when
        val nicknameState = NicknameState.validate("")
    ...
    fun `닉네임 첫 글자에 공백이 오면 유효하지 않은 형식이다`(blankFirstString: String) {
        // when
        val nicknameState = NicknameState.validate(blankFirstString)
    ...
    fun `닉네임에 밑줄, 마침표를 제외한 다른 특수기호가 포함되어 있다면 유효하지 않은 형식이다`(invalidNickname: String) {
        // when
        val nicknameState = NicknameState.validate(invalidNickname)
    ...
    fun `닉네임의 길이는 1자 이상 10자 이내이며, 10자를 초과하면 유효하지 않은 길이이다`() {
        ...
        val nicknameState = NicknameState.validate("엄청 기이이인 닉네임")
