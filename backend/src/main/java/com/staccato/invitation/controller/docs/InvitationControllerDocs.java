...
public interface InvitationControllerDocs {
    ...
    public ResponseEntity<CategoryInvitationSentResponses> readSentInvitations(
    ...
    @Operation(summary = "사용자가 요청받은 초대 목록 조회", description = "특정 사용자가 받은 모든 요청 상태의 초대 목록을 최신순으로 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(description = "받은 초대 목록 조회 성공", responseCode = "200")
    })
    public ResponseEntity<CategoryInvitationReceivedResponses> readReceivedInvitations(
            @Parameter(hidden = true) @LoginMember Member member);
}
