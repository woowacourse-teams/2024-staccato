...
public record MemberSearchResponse(
        ...
        @Schema(description = "검색된 사용자가 카테고리에 이미 속한 사용자인지, 초대 요청을 받은 사용자인지, 그 외인지를 나타냅니다. (ALREADY_JOINED/ALREADY_REQUESTED/NONE)", example = SwaggerExamples.SEARCHED_MEMBER_STATUS)
        String status
) {
    public static MemberSearchResponse requested(Member member) {
        return new MemberSearchResponse(member.getId(), member.getNickname()
                .getNickname(), member.getImageUrl(), SearchedStatus.ALREADY_REQUESTED.name());
    }

    public static MemberSearchResponse joined(Member member) {
        return new MemberSearchResponse(member.getId(), member.getNickname()
                .getNickname(), member.getImageUrl(), SearchedStatus.ALREADY_JOINED.name());
    }
