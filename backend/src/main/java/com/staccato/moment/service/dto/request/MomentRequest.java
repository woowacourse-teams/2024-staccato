...
@Schema(description = "순간 기록 생성 시 요청 형식입니다. 단, 멀티파트로 보내는 사진 파일은 여기에 포함되지 않습니다.")
public record MomentRequest(
        @Schema(example = "런던 박물관")
        @NotBlank(message = "순간한 장소의 이름을 입력해주세요.")
        @Size(min = 1, max = 30, message = "순간한 장소의 이름은 공백 포함 1자 이상 30자 이하로 설정해주세요.")
        String placeName,
        @Schema(example = "Great Russell St, London WC1B 3DG")
        @NotNull(message = "순간한 장소의 주소를 입력해주세요.")
        String address,
        @Schema(example = "51.51978412729915")
        @NotNull(message = "순간한 장소의 위도를 입력해주세요.")
        BigDecimal latitude,
        @Schema(example = "-0.12712788587027796")
        @NotNull(message = "순간한 장소의 경도를 입력해주세요.")
