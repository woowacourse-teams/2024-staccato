...
public interface MomentControllerDocs {
    ...
    ResponseEntity<MomentDetailResponse> readMomentById(
            @Parameter(hidden = true) Member member,
            @Parameter(description = "방문 기록 ID", example = "1", required = true) @PathVariable @Min(value = 1L, message = "방문 기록 식별자는 양수로 이루어져야 합니다.") long momentId);
    ...
    ResponseEntity<Void> updateMomentById(
            @Parameter(hidden = true) Member member,
            @Parameter(description = "방문 기록 ID", example = "1", required = true) @PathVariable @Min(value = 1L, message = "방문 기록 식별자는 양수로 이루어져야 합니다.") long momentId,
            @Parameter(description = "key = visitImageFiles") @Size(max = 5, message = "사진은 5장까지만 추가할 수 있어요.") List<MultipartFile> momentImageFiles,
    ...
    ResponseEntity<Void> deleteMomentById(
            @Parameter(hidden = true) Member member,
            @Parameter(description = "방문 기록 ID", example = "1", required = true) @Min(value = 1L, message = "방문 기록 식별자는 양수로 이루어져야 합니다.") long momentId
    );
}
