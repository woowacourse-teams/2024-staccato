...
public record MemoryDetailResponse(
    ...
    public MemoryDetailResponse(Memory memory, List<Moment> moments) {
        this(
            memory.getId(),
    ...
    private static List<MomentResponse> toMomentResponses(List<Moment> moments) {
        return moments.stream().map(MomentResponse::new).toList();
    }
}
